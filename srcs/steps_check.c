...
/*   Updated: 2019/03/11 19:51:06 by lsandor-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_step_forward_check(t_wolf *w, unsigned char flag)
{
	if (flag == 1)
	{
		w->temp = w->map.map[(int)(w->pl.pos.x + w->pl.dir.x * w->ms*3) + (int)w->pl.pos.y * w->map.map_w];
		if (!w->temp || w->temp  == 20)
			return (1);
	}
	else if (flag == 0)
	{
		w->temp = w->map.map[(int)(w->pl.pos.y + w->pl.dir.y * w->ms*3) * w->map.map_w + (int)w->pl.pos.x];
		if (!w->temp || w->temp  == 20)
			return (1);
	}
	return (0);
