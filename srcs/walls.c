...
/*   Updated: 2019/03/15 23:45:09 by jblack-b         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "wolf3d.h"

static void			ft_wall_hit_switch(t_thread *a)
{
	if (a->w.pl.side == 0)
...
void				ft_wall_hit(t_thread *a)
{
	...
	while (a->w.hit == 0)
	{
		...
		if (a->w.map.map[a->w.map.x + a->w.map.y * a->w.map.map_w] > 0\
		&& a->w.map.map[a->w.map.x + a->w.map.y * a->w.map.map_w] < 20)
			a->w.hit = 1;
	}
...
void				ft_wall_draw_start(t_thread *a)
{
	...
	a->w.texture_num =\
	a->w.map.map[a->w.map.x + a->w.map.y * a->w.map.map_w] - 1;
	if (a->w.pl.side == 0)
